...
export default function DatabaseBrowser({ project, collection, onSelectCollection, onOpenCreate, onOpenView, onOpenEdit, onDeleteDocument }) {
  ...
  const [connectionType, setConnectionType] = useState('fallback');
    ...
    async function fetchCollections() {
      try {
        ...
        if (data.success) {
          ...
          setConnectionType(data.connectionType || 'fallback');
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-800/60 text-xs">
              {loading ? (
                ...
                <tr>
                  <td colSpan={headers.length + 1} className="py-12 text-center text-amber-400 font-mono text-xs">
                    ⚠️ Notice: {fetchError}
                  </td>
                </tr>
              ) : documents.length === 0 ? (
